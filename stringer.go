...
func main() {
	...
	g.Printf(ENUMER, "// Code generated by \"enumer %s\"; DO NOT EDIT.\n", strings.Join(os.Args[1:], " "))
	g.Printf(ENUMER, "\n")
	if comments.String() != "" {
		g.Printf(ENUMER, "// %s\n", comments.String())
	}
	g.Printf(ENUMER, "package %s", g.pkg.name)
	g.Printf(ENUMER, "\n")
	g.Printf(ENUMER, "import (\n")
	g.Printf(ENUMER, "\t\"fmt\"\n")
	g.Printf(ENUMER, "\t\"strings\"\n")
	if *sql {
		g.Printf(ENUMER, "\t\"database/sql/driver\"\n")
	}
	if *json {
		g.Printf(ENUMER, "\t\"encoding/json\"\n")
	}
	if *gqlgen {
		g.Printf(ENUMER, "\t\"io\"\n")
		g.Printf(ENUMER, "\t\"strconv\"\n")
	}
	g.Printf(ENUMER, ")\n")
...
type Generator struct {
	enumerBuf, oapiEnumBuf, oapiMethodBuf bytes.Buffer // Accumulated output.
	pkg                                   *Package     // Package we are scanning.
}

// Printf prints the string to the output
func (g *Generator) Printf(bufType BufferType, format string, args ...interface{}) {
	switch bufType {
	case ENUMER:
		_, _ = fmt.Fprintf(&g.enumerBuf, format, args...)
	case OAPI_ENUM:
		_, _ = fmt.Fprintf(&g.oapiEnumBuf, format, args...)
	case OAPI_METHOD:
		_, _ = fmt.Fprintf(&g.oapiMethodBuf, format, args...)
	default:
		log.Fatalf("unknown buffer type: %v", bufType)
	}
}
...
func (g *Generator) format(bufType BufferType) []byte {
	var src []byte
	var err error

	switch bufType {
	case ENUMER:
		src, err = format.Source(g.enumerBuf.Bytes())
	case OAPI_ENUM:
		src, err = format.Source(g.oapiEnumBuf.Bytes())
	case OAPI_METHOD:
		src, err = format.Source(g.oapiMethodBuf.Bytes())
	default:
		log.Fatalf("unknown buffer type: %d", bufType)
	}
	if err != nil {
		// Should never happen, but can arise when developing this code.
		// The user can compile the output to see the error.
		log.Printf("warning: BufferType: %d, internal error: invalid Go generated: %s", bufType, err)
		log.Printf("warning: BufferType: %d,  compile the package to analyze the error", bufType)
		switch bufType {
		case ENUMER:
			return g.enumerBuf.Bytes()
		case OAPI_ENUM:
			return g.oapiEnumBuf.Bytes()
		case OAPI_METHOD:
			return g.oapiMethodBuf.Bytes()
		default:
			log.Fatalf("unknown buffer type: %d", bufType)
		}
	}
...
func (g *Generator) declareIndexAndNameVar(run []Value, typeName string) {
	...
	g.Printf(ENUMER, "const %s\n", n)
	g.Printf(ENUMER, "var %s\n", index)
...
func (g *Generator) declareNameVars(runs [][]Value, typeName string, suffix string) {
	g.Printf(ENUMER, "const _%sName%s = \"", typeName, suffix)
	for _, run := range runs {
		for i := range run {
			g.Printf(ENUMER, "%s", run[i].name)
		}
	}
	g.Printf(ENUMER, "\"\n")
	g.Printf(ENUMER, "const _%sLowerName%s = \"", typeName, suffix)
	for _, run := range runs {
		for i := range run {
			g.Printf(ENUMER, "%s", strings.ToLower(run[i].name))
		}
	}
	g.Printf(ENUMER, "\"\n")
}
...
func (g *Generator) buildOneRun(runs [][]Value, typeName string) {
	...
	if values[0].value == 0 { // Signed or unsigned, 0 is still 0.
		g.Printf(ENUMER, stringOneRun, typeName, usize(len(values)), lessThanZero)
	} else {
		g.Printf(ENUMER, stringOneRunWithOffset, typeName, values[0].String(), usize(len(values)), lessThanZero)
	}
}
...
func (g *Generator) buildMultipleRuns(runs [][]Value, typeName string) {
	...
	g.Printf(ENUMER, "func (i %s) String() string {\n", typeName)
	g.Printf(ENUMER, "\tswitch {\n")
	for i, values := range runs {
		if len(values) == 1 {
			g.Printf(ENUMER, "\tcase i == %s:\n", &values[0])
			g.Printf(ENUMER, "\t\treturn _%sName_%d\n", typeName, i)
			continue
		}
		g.Printf(ENUMER, "\tcase %s <= i && i <= %s:\n", &values[0], &values[len(values)-1])
		if values[0].value != 0 {
			g.Printf(ENUMER, "\t\ti -= %s\n", &values[0])
		}
		g.Printf(ENUMER, "\t\treturn _%sName_%d[_%sIndex_%d[i]:_%sIndex_%d[i+1]]\n",
			typeName, i, typeName, i, typeName, i)
	}
	g.Printf(ENUMER, "\tdefault:\n")
	g.Printf(ENUMER, "\t\treturn fmt.Sprintf(\"%s(%%d)\", i)\n", typeName)
	g.Printf(ENUMER, "\t}\n")
	g.Printf(ENUMER, "}\n")
}
...
func (g *Generator) buildMap(runs [][]Value, typeName string) {
	...
	for _, values := range runs {
		for _, value := range values {
			g.Printf(ENUMER, "\t%s: _%sName[%d:%d],\n", &value, typeName, n, n+len(value.name))
			n += len(value.name)
		}
	}
	g.Printf(ENUMER, "}\n\n")
	g.Printf(ENUMER, stringMap, typeName)
}
...
func (g *Generator) buildNoOpOrderChangeDetect(runs [][]Value, typeName string) {
	...
	`)
	g.Printf(ENUMER, "func _%sNoOp (){ ", typeName)
	g.Printf(ENUMER, "\n var x [1]struct{}\n")
	for _, values := range runs {
		for _, value := range values {
			g.Printf(ENUMER, "\t_ = x[%s-(%s)]\n", value.originalName, value.str)
		}
	}
	g.Printf(ENUMER, "}\n\n")
}
